...
// ---------------------------------------------------------------------------
// V18 ablation: GPU softmax kernel. The ONNX model emits 2-channel logits into
// a device buffer; the historical "CPU softmax" path copies both channels to
// the host and runs a per-voxel exp() reduction there. This kernel computes the
// foreground probability (softmax channel 1) on the device, so only ONE channel
// crosses PCIe (halving the D2H copy) and the host exp() loop disappears.
//   d_logits : device, [B, 2, patch_vox] FP32 (channel-major within a patch)
//   d_prob   : device, [B, patch_vox]    FP32 (caller-allocated)
// Operates on the default stream; the caller's D2H cudaMemcpy is ordered after.
__global__ void softmax_ch1_kernel(const float* __restrict__ logits,
                                   float* __restrict__ prob,
                                   long long patch_vox, int B) {
    long long i = blockIdx.x * (long long)blockDim.x + threadIdx.x;
    long long total = patch_vox * (long long)B;
    if (i >= total) return;
    int b = (int)(i / patch_vox);
    long long v = i - (long long)b * patch_vox;
    const float* l = logits + (long long)b * 2 * patch_vox;
    float l0 = l[v];
    float l1 = l[v + patch_vox];
    float m  = l0 > l1 ? l0 : l1;
    // Precise expf (not the fast __expf intrinsic) so the device softmax is
    // numerically faithful to the host std::exp path -> identical Dice.
...
void cuda_softmax_ch1(const float* d_logits, float* d_prob,
    ...
    softmax_ch1_kernel<<<(unsigned)blocks, threads>>>(d_logits, d_prob,
                                                      patch_vox, B);
}

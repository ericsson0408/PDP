...
struct Args {
    ...
    bool  unet25d = false;     // V16-2/V17-2: use 2.5D slice-by-slice inference
    ...
    // V18 ablation toggles (default = current optimized pipeline, unchanged):
    bool no_trt = false;                // --no-trt: force CUDA(cuDNN) EP even under v16
    bool cudnn_heuristic = false;       // --cudnn-heuristic: lightweight cuDNN search
    bool gpu_softmax = false;           // --gpu-softmax: device-side softmax (non-overlap)
    bool serial_softmax = false;        // --serial-softmax: no OpenMP on host softmax (naive)
...
static Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (s == "--v8" || s == "--classical") {   // --classical: readable alias for the classical pipeline
        ...
        else if (s == "--v12-no-overlap" || s == "--no-stream-overlap") a.v12_overlap = false;  // --no-stream-overlap: readable alias
        else if (s == "--trt-cache") a.trt_cache = next();
        else if (s == "--trt-no-fp16") a.trt_fp16 = false;
        else if (s == "--no-trt") a.no_trt = true;
        else if (s == "--cudnn-heuristic") a.cudnn_heuristic = true;
        else if (s == "--gpu-softmax") a.gpu_softmax = true;
        else if (s == "--serial-softmax") a.serial_softmax = true;
        else if (s == "--v14") {
        ...
        else if (s == "--v16" || s == "--gate") {   // --gate: readable alias for the dynamic-threshold gate post-processor
        ...
        else if (s == "--v17" || s == "--v17-hybrid" || s == "--hybrid") {   // --hybrid: readable alias for the oracle/actuator hybrid post-processor
        ...
        else if (s == "--unet25d" || s == "--model-2p5d") {   // --model-2p5d: readable alias for 2.5D slice-by-slice inference
            // 2.5D slice-by-slice inference using model/airway_2p5d_unet.onnx.
            // Composes with --gate (--v16) or --hybrid (--v17) for the full
            // 2.5D / 2.5D-hybrid post-processing pipeline.
            // Does NOT enable v12 (TRT/stream-overlap) -- unnecessary for 2D ops.
            a.unet25d = true;
            a.v9_1    = true;   // trigger ONNX inference path
            a.v14     = true;   // use central-CC post-processing
            a.onnx_model = "model/airway_2p5d_unet.onnx";  // 2.5D model (override with --onnx-model)
            a.onnx_batch = 16;  // larger batch is cheap for 2D slices
            a.onnx_thr   = 0.5f;
        }
        else if (s == "--fast") {
...
int main(int argc, char** argv) {
    ...
    if (rank == 0 && (a.v9_1 || a.v9_2)) {
        ...
        OrtOptions oo;
        oo.use_trt        = a.v12 && !a.no_trt;   // TRT EP under --v12, unless --no-trt forces cuDNN
        oo.trt_fp16       = a.trt_fp16;
        oo.trt_engine_cache = true;
        oo.trt_cache_path = a.trt_cache;
        oo.stream_overlap = a.v12 && a.v12_overlap && !a.no_trt;
        oo.cudnn_heuristic = a.cudnn_heuristic;   // V18 ablation
        oo.gpu_softmax     = a.gpu_softmax;       // V18 ablation (non-overlap path)
        oo.serial_softmax  = a.serial_softmax;    // V18 ablation (naive baseline)
        ...
        if (do_resample) {
            ...
            printf("[25d] 2.5D slice-by-slice inference  slices=%d  batch=%d  "
                   "hu=[%.0f,%.0f]\n", nz, a.onnx_batch, a.onnx_hu_lo, a.onnx_hu_hi);
            ms = ort_infer_volume_25d(sess, vol.data, unet_prob.data(),
                                      nx, ny, nz,
                                      a.onnx_hu_lo, a.onnx_hu_hi,
                                      a.onnx_batch);
        } else {
